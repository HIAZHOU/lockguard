...
**这个 App 收集位置与运动数据，但全部只在本机使用，不会上传到任何地方。**
它在 `AndroidManifest.xml` 里**没有声明 `INTERNET` 权限**，因此从系统层面就无法建立网络连接。
...
- 诊断日志（「更多 → 分享诊断」）由你主动触发导出，**不含坐标**，只含权限状态、机型与识别原因。你可以自行决定是否分享。
- `android:allowBackup="false"`：App 数据不参与系统云备份。

## 三、会不会上传、会不会分享给第三方

- **不会上传。** App 未申请网络权限，代码中没有网络请求逻辑，也没有集成任何统计、广告、崩溃上报或支付 SDK。
- **不会分享给第三方。** 不存在数据传输，也就没有接收方。
...
| 权限 | 为什么需要 | 不给会怎样 |
|---|---|---|
| `ACCESS_FINE_LOCATION` / `ACCESS_COARSE_LOCATION` | 判断骑行与步行 | 完全无法工作 |
| `ACCESS_BACKGROUND_LOCATION` | 手机在口袋/息屏时仍需判断 | 只在打开 App 时有效 |
...
## 五、关于位置信息（敏感个人信息）的单独告知

根据《中华人民共和国个人信息保护法》，位置信息属于**敏感个人信息**，处理前需取得你的单独同意。本 App 的处理方式是：

- 处理目的：仅用于在本地判断「骑行—停车—走开」，触发锁车确认提醒。
- 处理方式：设备本机实时计算，不存储、不传输。
- 必要性：这是本 App 唯一的核心功能，不处理位置就无法工作。
- 拒绝后果：你可以不授予位置权限，App 将无法运行（可随时卸载）。

**你不授予权限即可不使用；授予后可随时在系统设置中撤回。卸载 App 即删除全部本机数据。**
...
本 App 使用的开源/官方库（Jetpack Compose、AndroidX、Google Play Services Location）均不包含数据上报行为。唯一与「收款二维码」相关的工具类（ZXing）仅用于你**本机构建时**从自己的收款码图片生成资源，不参与 App 运行。
...
- The app **does not declare the `INTERNET` permission**, so it physically cannot send data anywhere.
- No analytics, ads, crash reporting, or payment SDKs are included.
- Location data is used for real-time computation only and is never written to disk or transmitted.
- Only your threshold settings are stored locally via DataStore. `android:allowBackup="false"`.
- Diagnostics logs are exported manually by you and contain **no coordinates**.
